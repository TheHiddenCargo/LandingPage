...
// Puedes definir un icono por defecto en caso de que data.imagePath no esté presente
const defaultIcon = blueGuy;

export const UserBar = ({ userNickname, email }) => {
        ...
        <div className="user-info">
          <div className="icon">
            <button onClick={handleUpdate}>
              <img
                className="icon"
                src={data.imagePath || defaultIcon}
                alt="User Icon"
              />
            </button>
          </div>
...
const UserDialog = ({ toCreate, currentNickname, email, updateNickname }) => {
  ...
  const { loading: loadingCreate, status: statusCreate, error: errorCreate } =
    useFetch(
      {
  ...
  const { data: userData, loading: userLoading, status: userStatus } = useFetch(
    {
      url:
        "https://thehiddencargo1.azure-api.net/creation/users/" +
        currentNickname +
        "/info",
      method: "GET",
      headers: {
  ...
  const { loading: nickLoading, status: nickStatus } = useFetch(
    {
  ...
  const { loading: iconLoading, status: iconStatus } = useFetch(
    {
  ...
  }, [loadingCreate, statusCreate, createUser, newNickname, updateNickname]);
  ...
  }, [userLoading, userData, userStatus, toCreate, updateUser, createUser]);

  /* Handle Update */
  useEffect(() => {
    console.log("UPDATING");
    console.log(nickLoading, iconLoading, nickStatus, iconStatus);
    if (!nickLoading && !iconLoading && nickStatus === 200 && iconStatus === 200)
  ...
  }, [nickLoading, iconLoading, nickStatus, iconStatus, currentNickname, userData]);
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="select-icons">
          {icons.map((iconMap, index) => (
            ...
            >
              <img src={iconMap} alt={`Icon ${index}`} />
            </div>
          ))}
        </div>
